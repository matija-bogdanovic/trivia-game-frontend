...
function AvatarCropper({ imageSrc, onDone, onCancel }: AvatarCropperProps) {
  ...
  return (
    ...
    >
      <div className="flex w-full max-w-md flex-col gap-4 rounded-lg border border-white/[0.07] bg-arena-800 p-6">
        <h3
          id="crop-title"
        ...
        <div className="flex flex-wrap justify-end gap-3">
          <button
            ...
            onClick={onCancel}
            disabled={busy}
            className="cursor-pointer rounded-lg border border-white/20 px-5 py-3 text-[10px] tracking-[0.2em] text-white uppercase transition-colors hover:bg-arena-700 focus-visible:ring-2 focus-visible:ring-gold focus-visible:outline-none disabled:opacity-50"
          >
            {t('profile.cancel')}
          </button>
          <button
