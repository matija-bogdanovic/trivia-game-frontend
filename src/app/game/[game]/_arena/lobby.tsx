...
export default function ArenaLobby() {
  ...
  return (
    <div className="flex h-full flex-col p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 text-[11px] tracking-wider text-arena-200">
            ...
            <span
              className={`flex items-center gap-1 rounded-lg border px-2 py-0.5 text-[9px] tracking-widest uppercase ${
                isPrivate
      ...
      {/* Connected bar */}
      <div className="mb-6 flex flex-wrap items-center gap-4 rounded-lg border border-white/[0.07] bg-arena-800 px-5 py-3">
        {/*
      ...
      {/*
        The room changed hands. The banner itself is shared with the match
        screen — see host_change_banner — because a handover can happen in
        either, and two copies would drift.

        Above the roster rather than inside it: it is about the room and not
        about a seat, and it explains the thing the reader is about to notice
        anyway, which is the START button appearing or vanishing.
      */}
      <HostChangeBanner className="mb-4" />
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-6 xl:flex-row">
          ...
          */}
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <button
              ...
              className={`inline-flex shrink-0 items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[10px] font-bold tracking-[0.15em] uppercase transition-colors focus-visible:ring-2 focus-visible:ring-gold focus-visible:outline-none ${
                roomFull
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {seated.map((player) => {
              ...
              return (
                <div
                  key={player.username}
                  className={`bg-arena-800 rounded-lg border p-5 relative ${
                    isMe ? 'border-gold/30' : 'border-white/[0.07]'
                  }`}
                >
                  {player.isHost && (
                    <div className="absolute top-3 right-3 text-[9px] tracking-widest text-gold rounded-lg border border-gold/40 px-2 py-0.5 uppercase">
                      {t('arena.lobby.host')}
                    </div>
                  )}
                  {isMe && !player.isHost && (
                    <div className="absolute top-3 right-3 text-[9px] tracking-widest text-arena-200 rounded-lg border border-arena-400 px-2 py-0.5 uppercase">
                      {t('arena.common.you')}
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div
                      className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1 text-[10px] font-bold tracking-widest uppercase ${
                        player.connected
                          ? 'border-gold/40 text-gold'
                  ...
                  */}
                  {iAmHost && !isMe && (
                    <button
                      ...
                      className="mt-3 flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg border border-arena-500 py-1.5 text-[10px] tracking-wider text-arena-300 uppercase transition-colors hover:border-arena-300 hover:text-white focus-visible:ring-2 focus-visible:ring-gold focus-visible:outline-none"
                    >
            ...
            */}
            {Array.from({ length: emptySlots }).map((_, i) => (
              <div
                key={`empty-${i}`}
                className="flex items-center justify-center rounded-lg border border-white/[0.04] bg-arena-750 p-5"
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            */}
            <button
              ...
              className="ml-auto rounded-lg border border-white/10 px-5 py-4 text-[11px] tracking-[0.15em] text-arena-200 uppercase transition-colors hover:bg-arena-700 hover:text-white focus-visible:ring-2 focus-visible:ring-gold focus-visible:outline-none"
            >
              {t('arena.lobby.leave')}
            </button>
            {iAmHost && (
        ...
        {/* Chat */}
        <div className="flex min-h-0 flex-col rounded-lg border border-white/[0.07] bg-arena-800 xl:w-72">
          ...
          <div className="border-t border-white/[0.07] p-3 flex gap-2">
            <input
              ...
              className="flex-1 bg-arena-750 rounded-lg border border-white/10 text-white text-xs px-3 py-2 outline-none focus:border-gold/30 placeholder:text-arena-400 min-w-0"
            />
            <button
              onClick={send}
      ...
      {confirmingKick && (
        ...
        >
          <div className="flex max-w-md flex-col gap-4 rounded-lg border border-gold/30 bg-arena-800 p-8">
