    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <header className="flex flex-wrap items-center justify-between gap-x-6 gap-y-3 border-b border-white/[0.07] bg-arena-950 px-4 py-3 sm:px-6">
        <div className="flex items-center gap-5">
          ...
          <ol className="hidden items-center gap-1.5 md:flex">
            {LADDER.map((label, i) => (
              <li
                key={label}
                className={`rounded-lg border px-2 py-1 text-[9px] tracking-widest uppercase transition-colors duration-300 ${
                  i < difficulty
        ...
        <div className="flex items-center gap-4">
          ...
          {spectating && (
            <div
              className="rounded-lg border border-gold/40 bg-gold/10 px-2.5 py-1 text-[9px] font-bold tracking-[0.2em] text-gold uppercase"
              role="status"
            >
          ...
          <button
            type="button"
            onClick={onLeave}
            className="cursor-pointer rounded-lg border border-arena-500 px-3 py-1.5 text-[10px] tracking-wider text-arena-300 uppercase transition-colors hover:border-arena-300 hover:text-white focus-visible:ring-2 focus-visible:ring-gold focus-visible:outline-none"
          >
            {t('arena.lobby.leave')}
          </button>
        </div>
          ...
          {players
            ...
            .map((p) => {
              ...
              return (
                <div
                  key={p.username}
                  className={`min-w-[9.5rem] rounded-lg border p-3 transition-colors duration-150 lg:min-w-0 ${
                    isAnswering
